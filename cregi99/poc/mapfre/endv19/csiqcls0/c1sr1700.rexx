...
/* NAME: C1SR1700                                                    */
/*                                                                   */
/* PURPOSE: THIS CLIST WILL DRIVE THE ARCHIVED PACKAGE REPORTS       */
/*          SUB-DIALOG                                               */
/*                                                                   */
/*-------------------------------------------------------------------*/
...
DISP: +
    ISPEXEC DISPLAY PANEL(C1SR1700)  +
                    CURSOR(&PNLCUR)  +
                    MSG   (&PNLMSG)
...
/* IF EDITS OK THEN SAVE VARIABLES                                  */
    ISPEXEC VPUT ( VARSPPKG RAPPROVE +
...
/*------------------------------------------------------------------*/
  ERROR -
    ...
    SET &RPTNUM = RPT56
    IF &STR(&RPT56) NE &STR( ) AND +
       &STR(&RPT56) NE &STR(_) THEN SET &REPORT = &STR(&REPORT 56)

    SET &RPTNUM = RPT57
    IF &STR(&RPT57) NE &STR( ) AND +
       &STR(&RPT57) NE &STR(_) THEN SET &REPORT = &STR(&REPORT 57)

    SET &RPTNUM = RPT58
    IF &STR(&RPT58) NE &STR( ) AND +
       &STR(&RPT58) NE &STR(_) THEN SET &REPORT = &STR(&REPORT 58)
  ...
  ERROR -
    DO
      ...
      ELSE -
        DO
          SET &ERROR = ON
          WRITE AN I/O ERROR OCCURRED IN C1SR1700. RC=&RC
          IF (&RC = 348) THEN -
            WRITE CHECK DEFAULTS VALUES ON OPTION 0 (SPACE ETC).
          GOTO FINI
        END
      RETURN
    END
