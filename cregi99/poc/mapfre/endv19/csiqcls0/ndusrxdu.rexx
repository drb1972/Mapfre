...
if ARG(1) == "DESCRIPTION" THEN
   RETURN "Use the 'DP' command to compare this Ele to the next up the map"
/* End of Preamble */
...
Execute_CSV_Action:
   ...
   ADDRESS TSO,
   "ALLOC FI(CSVMSGS1) BLKSIZE(0) TRACKS LRECL(133) SPACE(5 5)",
    "DSORG(PS)",
       "RECFM(F B A) NEW REUSE UNCATALOG" ;
...
If Match_Found == 'NO' then               /* check we found the ele */
   ADDRESS ISPEXEC "SETMSG MSG(NDUM019C)" /* warn user, refresh needed     */
/*
