...
//*--------------------------------------------------------- ENXSJCL --*
//*  Deploy to &TOSUBSYS from &FROMSUBS
//*--------------------------------------------------------------------*
//*--------------------------------------------------------------------*
//*   STEP 1 -- Execute CSV Utility to locate inventory
//*--------------------------------------------------------------------*
//STEP1    EXEC PGM=NDVRC1,REGION=4096K,
...
//*--------------------------------------------------------------------
//SHOWME  EXEC PGM=IEBGENER,REGION=1024K,COND=(4,LT,STEP1)
//SYSPRINT DD SYSOUT=*
...
//*--------------------------------------------------------------------
//SCANTARG EXEC PGM=IRXJCL,PARM='ENBPIU00 PARMLIST',
//          COND=(4,LT,STEP1)
...
'&&FULL_ELM_NAME'
     FROM ENVIRONMENT &&ENV_NAME
          ...
          TYPE        &&TYPE_NAME
     OPTIONS
          CCID  'CLEANUP' COMMENT 'DEPLOY4TEST CLEANUP'
                OVERRIDE SIGNOUT
     .
//OPTIONSD DD *
...
//*--------------------------------------------------------------------*
//*  Cleanup previous Deploy for Test executions
//*--------------------------------------------------------------------*
//NDVRBAT  EXEC PGM=NDVRC1,DYNAMNBR=1500,REGION=4096K,
//  PARM='C1BM3000',COND=((0,NE,SCANTARG),(4,LT))
)IM SCMM@LIB
...
//*--------------------------------------------------------------------*
)SEL &ACTION EQ DEPLOY
//*--------------------------------------------------------------------*
//*- Cast and Execute the XFER package
//*--------------------------------------------------------------------*
//ENBP1000 EXEC PGM=NDVRC1,COND=(4,LT),
//             PARM=ENBP1000,
//             DYNAMNBR=1500,
//             REGION=4096K
)IM SCMM@LIB
...
//* ENXSJCL  WAS GENERATED BY &ZUSER ON &ZDATE AT &ZTIME
